...
- [**`discord_switch.bat`**](./discord_switch.bat) - включение/выключение обхода Discord, если у кого-то новый способ обхода не работает *(на деле перетирает список `ipset-discord`, перезапуск после изменения не нужен)*
